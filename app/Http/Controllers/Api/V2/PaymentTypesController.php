...
class PaymentTypesController
{
    ...
    public function getList(Request $request)
    {
        ...
        if ( get_setting('orange_payment') == 1) {
            $payment_type = array();
            $payment_type['payment_type'] = 'orange_payment';
            $payment_type['payment_type_key'] = 'orange';
            $payment_type['image'] = static_asset('assets/img/cards/orange.png');
            $payment_type['name'] = "Orange money";
            $payment_type['title'] = translate("Payer Orange money");
            if($mode == 'wallet') {
                $payment_type['title'] = "Recharger avec Orange money";
            }

            $payment_types[] = $payment_type;
        }
        if ( get_setting('coris_payment') == 1) {
            $payment_type = array();
            $payment_type['payment_type'] = 'coris_payment';
            $payment_type['payment_type_key'] = 'coris';
            $payment_type['image'] = static_asset('assets/img/cards/coris.png');
            $payment_type['name'] = "Coris money";
            $payment_type['title'] = translate("Payer avec Coris money");
            if($mode == 'wallet') {
                $payment_type['title'] = "Recharger avec Coris money";
            }

            $payment_types[] = $payment_type;
        }

        if ( get_setting('moov_payment') == 1) {
            $payment_type = array();
            $payment_type['payment_type'] = 'moov_payment';
            $payment_type['payment_type_key'] = 'moov';
            $payment_type['image'] = static_asset('assets/img/cards/moov.png');
            $payment_type['name'] = "Moov money";
            $payment_type['title'] = translate("Payer avec Moov money");
            if($mode == 'wallet') {
                $payment_type['title'] = "Recharger avec Moov money";
            }
        ...
        if ( get_setting('paypal_payment') == 1) {
            ...
            $payment_type['title'] = "Payer avec Paypal";
            if($mode == 'wallet') {
                $payment_type['title'] = "Recharger avec Paypal";
            }
        ...
        if (get_setting('stripe_payment') == 1) {
            ...
            $payment_type['title'] = "Payer avec Visa";
            if($mode == 'wallet') {
                $payment_type['title'] = "Recharger avec Visa";
            }
