...
class AuthController extends Controller
{
    public function signup(Request $request)
    {
        if (User::where('email', $request->email_or_phone)->orWhere('phone', $request->email_or_phone)->first() != null) {
            return response()->json([
                'result' => false,
                'message' => 'Adresse email ou numéro de téléphone déjà utilisé, veuillez entrez un autre.',
                'user_id' => 0
            ], 201);
        }
        ...
        return response()->json([
            'result' => true,
            'message' => 'Inscription réussie. Veuillez vérifier vos messages (SMS ou emails).',
            'user_id' => $user->id
        ], 201);
    }
    ...
    public function resendCode(Request $request)
    {
        ...
        return response()->json([
            'result' => true,
            'message' => 'Le code de vérification a été renvoyé',
        ], 200);
    }
    ...
    public function confirmCode(Request $request)
    {
        ...
        if ($user->verification_code == $request->verification_code) {
            ...
            return response()->json([
                'result' => false,
                'message' => 'Code incorrect, vous pouvez demander un autre',
            ], 200);
        }
    }
        ...
        if ($user != null) {
            if (Hash::check($request->password, $user->password)) {
                ...
                if ($user->email_verified_at == null) {
                    return response()->json(['message' => 'Veuillez vérifier votre compte', 'user' => null], 401);
                }
            ...
            return response()->json(['result' => false, 'message' => 'Compte inexistant', 'user' => null], 401);
        }
